...
const newsResponse = z.object({
  news: z.array(z.object({ id: z.string(), title: z.string(), summary: z.string(), link: z.string(), translateLink: z.string(), pubDate: z.string(), source: z.string(), sourceKo: z.string(), category: z.string(), isKorean: z.boolean(), isBlog: z.boolean().optional(), image: z.string().optional() })),
  sources: z.array(z.object({ name: z.string(), ok: z.boolean(), checkedAt: z.string().nullable() })),
...
export default function NewsPage() {
  ...
  const fetchNewsData = () => {
    ...
    ky.get('/api/news', { timeout: 45000, retry: 0, cache: 'no-store', throwHttpErrors: false }).json()
      .then((raw) => {
        const data = newsResponse.parse(raw);
        const failed = new Set(data.sources.filter((entry) => !entry.ok).map((entry) => entry.name));
        setNews((previous) => [...data.news, ...previous.filter((item) => failed.has(item.sourceKo))]);
        setError(failed.size ? '일부 소스 조회 실패: ' + [...failed].join(', ') + ' · 해당 소스는 이전 결과가 있으면 표시합니다.' : '');
        if (data.updatedAt) setUpdatedAt(data.updatedAt);
      })
      .catch((failure: unknown) => {
        setError(failure instanceof Error ? '뉴스 조회 실패 · 이전 수집 결과가 있으면 표시합니다.' : '뉴스 응답을 확인하지 못했습니다.');
      })
  ...
  return (
    <div className="w-full">
        ...
        <p className="text-gray-500">
          ...
          {updatedAt && (
            <span className="ml-2 text-xs text-gray-600">
              · 마지막 수집 성공: {new Date(updatedAt).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' })}
            </span>
          )}
        </p>
      </div>

      <div className="flex flex-wrap gap-3 items-center justify-between mb-6 border-b border-white/5 pb-4">
      ...
      {error && tab !== 'youtube' && <p role="alert" className="mb-4 rounded-lg border border-amber-400/30 p-4 text-sm text-amber-200">{error}</p>}
        ...
        <>
          ...
          {loading ? (
            ...
            <div className="text-center py-20 text-gray-500">
              ...
              <p>{error ? '뉴스를 수집하지 못했습니다. 잠시 후 새로고침해주세요.' : '선택한 조건에 맞는 뉴스가 없습니다.'}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map((item) => (
