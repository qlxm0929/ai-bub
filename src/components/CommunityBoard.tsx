...
const CATEGORIES = ['전체', '💡 AI 팁', '🔮 프롬프트 공유', '📰 뉴스/발견', '❓ 질문', '🛠️ 도구 추천', '💬 자유'];
...
export default function CommunityBoard() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
      {/* 헤더 */}
      <div className="mb-10">
        <div className="badge badge-purple mb-3">커뮤니티 게시판</div>
      ...
      {/* 글쓰기 폼 */}
      {showForm && (
        <div ref={formRef} className="glass-card p-6 mb-8 border border-purple-500/20" style={{ background: 'rgba(124,58,237,0.05)' }}>
          <h2 className="text-lg font-bold text-white mb-5 flex items-center gap-2">✏️ 새 글 작성</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  maxLength={20}
                  placeholder="익명의 탐험가"
                  className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-purple-500/50 transition-colors"
                  required
                />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea
                ...
                maxLength={1000}
                rows={5}
                placeholder="AI 팁, 프롬프트, 발견한 정보 등을 자유롭게 공유해주세요!"
                className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-purple-500/50 transition-colors resize-none"
                required
              />
            </div>
