...
import { tools, categories, quickFilters, Tool } from '@/lib/tools';
import { NewTool } from '@/lib/newtools';
import { useState } from 'react';

interface Props {
  initialNewTools: NewTool[];
  fetchFailed: boolean;
}

export default function ToolsClient({ initialNewTools, fetchFailed }: Props) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="mb-8 flex flex-wrap gap-2">
        ...
        {(query || activeFilter) && (
          <span className="self-center text-xs text-gray-500 ml-1">{totalFiltered}개 도구</span>
        )}
      </div>

      <div className="space-y-16">
        {/* 최신 AI 도구 (Product Hunt) — 서버에서 미리 가져온 데이터 */}
        {!activeFilter && (
          <section>
            ...
            {isRefreshing ? (
              ...
              <div className="text-center py-10 text-gray-600 border border-white/5 rounded-2xl">
                ...
                <button onClick={refreshNewTools} className="mt-3 px-4 py-1.5 text-xs bg-white/5 hover:bg-white/10 rounded-lg transition-colors">
                  다시 시도
                </button>
              </div>
        ...
        {/* 검색/필터 결과 없음 */}
        {totalFiltered === 0 && (query || activeFilter) && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {categoryTools.map((tool, i) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-400 leading-relaxed mb-5">{tool.description}</p>
